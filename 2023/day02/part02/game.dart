...
class Game {
  ...
  int get power {
    int minR = this._roundList.map((round) => round.r).toList().reduce(max);
    int minG = this._roundList.map((round) => round.g).toList().reduce(max);
    int minB = this._roundList.map((round) => round.b).toList().reduce(max);
