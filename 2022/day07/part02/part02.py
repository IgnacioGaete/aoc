...
def main(argv):
    ...
    sh = Shell(file_path)
    sh.ls_all()
    space_needed = sh.get_space_needed_for_update()
    print(f"space needed: {space_needed}")
    result = sh.find("d", "/", lambda dir: dir.get_size() >= space_needed)
    result.sort(key=lambda dir: dir.get_size())
    print("\n-------------------------\n")
    print(result[0])
...
class Shell:
    def __init__(self, file_path):
        ...
        self.__max_disc_space = 70_000_000
        self.__update_space_required = 30_000_000
    ...
    def get_space_available(self):
        return self.__max_disc_space - self.__root.get_size()

    def get_space_needed_for_update(self):
        return max(self.__update_space_required - self.get_space_available(), 0)
